            ...
            {!loading && revisionData.length > 0 && (
                <div className="space-y-6">
                    {revisionData.map((item, idx) => (
                        <div key={idx} className="bg-[#111827] border border-gray-800 p-6 rounded-xl space-y-4 shadow-sm hover:border-gray-700 transition-colors">
                            <h3 className="text-white font-bold text-xl">{item.topic}</h3>
                            
                            <div className="bg-gray-900/50 p-4 rounded-lg border border-gray-800">
                                <h4 className="text-gray-400 text-xs font-bold uppercase tracking-widest mb-2">Key Notes</h4>
                            ...
                            {item.videos && item.videos.length > 0 && (
                                <div className="pt-2">
                                     <h4 className="text-gray-400 text-xs font-bold uppercase tracking-widest mb-3">Recommended Videos</h4>
                                     <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                         {item.videos.map((vid, vidIdx) => (
                                             <a 
                                                ...
                                                className="flex items-center gap-3 p-3 bg-gray-800/40 hover:bg-gray-800 border border-gray-700/50 hover:border-gray-600 rounded-lg group transition-all"
                                             >
                                                <div className="bg-red-500/10 p-2.5 rounded-md group-hover:bg-red-500/20 transition-colors">
                                                    <svg className="w-5 h-5 text-red-500" fill="currentColor" viewBox="0 0 24 24">
                                                        <path d="M19.615 3.184c-3.604-.246-11.631-.245-15.23 0-3.897.266-4.356 2.62-4.385 8.816.029 6.185.484 8.549 4.385 8.816 3.6.245 11.626.246 15.23 0 3.897-.266 4.356-2.62 4.385-8.816-.029-6.185-.484-8.549-4.385-8.816zm-10.615 12.816v-8l8 3.993-8 4.007z"/>
                                                    </svg>
                                                </div>
                                                <span className="text-sm font-medium text-gray-200 group-hover:text-white transition-colors">{vid.title}</span>
                                             </a>
                                         ))}
                                     </div>
                                </div>
